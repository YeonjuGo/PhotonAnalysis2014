...
void fakerate(){
    ...
    TFile* readFile = new TFile("yskim_pA_Pyquen_allQCDPhoton_hiForest2_v85.root" ,"READ");
...
//
    int nJetmax = 10;
    float refPt[nJetmax], hf4Sum, pt[nJetmax];
    ...
    for (int icent = 1 ; icent <= nCentBinPa ; icent++){
        for (int i = 0; i<nPtBin ; i++){
            ...
            for(int k=0; k<entries; k++){
                yJet -> GetEntry(k);
                if (Form("refPartonFlv >-100 && (pt >= %d && pt < %d) && (hf4Sum > %f && hf4Sum <= %f)", (int)ptBin[i], (int)ptBin[i+1], (float)centBinPa[icent-1], (float)centBinPa[icent]));
