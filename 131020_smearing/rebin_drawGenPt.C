...
TH1D* rebin(TH1D* h1, TH1D* h2){
    for (int i = 0; i < h1 -> GetNbinsX() ; i++){
        int yy = h1 -> GetBinContent(i);
        int xx = h1 -> GetBinCenter(i);
        h2 -> Fill(xx, yy);
    }   
    return h2; 
}
void rebin_drawGenPt(int fixOpt=0){
    TH1::SetDefaultSumw2();
...
//  
    ...
    TCanvas* cGenPt = new TCanvas("cGenPt", "", 700, 800);
    cGenPt -> Divide(1,2);

    TH1D* hGenPt[5];
   // TH1D* hGenBR[5];//hGenBeforeRebin
    TH1D* hGenPt_ratio[5];
    for(int icoll=0; icoll<5; icoll++){
      //  hGenBR[icoll]= new TH1D(Form("hGenBR%d",icoll), "; p_{T}^{GenJet} (GeV) ; Normalized Entries", 20, 0, 200);
        hGenPt[icoll] = new TH1D(Form("hGenPt%d",icoll),"; p_{T}^{GenJet} (GeV) ; Normalized Entries", 14, ptbin);
    }
    ...
    for(int icoll=1; icoll<5; icoll++){
        ...
        } else hGenPt_ratio[icoll] -> Draw("same");
    }
    jumSun(0,1,200,1,2);
  ...
  //  TH1D* hRecoBR[5];
   ...
   //     hRecoPt[icoll]= new TH1D(Form("hRecoPt%d",icoll), "; p_{T}^{RecoJet} (GeV) ; Normalized Entries", 20, 0, 200);
        hRecoPt[icoll] = new TH1D(Form("hRecoPt%d", icoll),"; p_{T}^{RecoJet} (GeV) ; Normalized Entries", 14, ptbin);
    }
...
//
    ...
    TH1D* hrg[5];
    for(int icoll=0; icoll<5; icoll++){
      //  hrg[icoll]= new TH1D(Form("hrg%d", icoll),"; p_{T} (GeV) ; p_{T}^{Reco}/p_{T}^{Gen}", 14, ptbin);
  ...
  //  makeMultiPanelCanvas(cSmear, 1, 3);
    ...
    TH1D* hRatioGen[5]; 
    for(int icoll=0; icoll<5; icoll++){    
        hSmear[icoll] = new TH1D(Form("hSmear%d",icoll), "; p_{T}^{Smeared} (GeV) ; Entries", 14, ptbin);
    }
