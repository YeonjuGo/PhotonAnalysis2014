...
void drawPtDependence( bool saveFigures=true) {
  ...
  for (int ipt=1 ; ipt<=nPtBin ; ipt++) {
    for (int icoll=0 ; icoll<6 ; icoll++) {
      ...
      char* fname =  Form("/home/goyeonju/recent2013/jetAnalysis/gammaJetAnalysis/histogramProducer/ffFiles/photonTrackCorr_%s_output_photonPtThr%d_to_%d_jetPtThr40_20131011.root",sampleName.Data(), (int)ptBin[ipt-1], (int)ptBin[ipt]);
