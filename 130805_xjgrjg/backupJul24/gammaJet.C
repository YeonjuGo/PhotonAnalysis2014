...
#include "corrFunctionGammaJet.h"
...
void gammaTrack(int sampleType = kHIDATA, float photonPtThr=60, float jetPtThr=30, int icent =1, TString jAlgo="akPu2PF", float coneDR=0.3, bool jetMixing=false) {
  TH1::SetDefaultSumw2();
  ...
  TString outName=  Form("ffFiles/photonTrackCorr_%s_output_photonPtThr%d_jetPtThr%d_jAlgo%s_coneDR%.2f_%d_jetMixing%d.root",stringSampleType.Data(),(int)photonPtThr, (int)jetPtThr, jAlgo.Data(), (float)coneDR, date->GetDate(), (int)jetMixing);
  delete date;
  ...
  */
  ...
  /////////// Tracks
  multiTreeUtil* tJet[3];
  tJet[kTrkRaw] = new multiTreeUtil();
  tJet[kTrkBkg] = new multiTreeUtil();
  
  if (!jetMixing) {
    tJet[kTrkRaw]->addFile(fname,  "yTrk",  evtSeltCut,  1);
    tJet[kTrkBkg]->addFile(fname,  "mTrk",  evtSeltCut,  1);
    tJet[kTrkRaw]->AddFriend("tgj");
    tJet[kTrkBkg]->AddFriend("tgj");
    }
  else {
    tJet[kTrkRaw]->addFile(fname,  "yTrkMb",  evtSeltCut,  1);
    tJet[kTrkBkg]->addFile(fname,  "mTrkMb",  evtSeltCut,  1);
    tJet[kTrkRaw]->AddFriend("tgjMb");
    tJet[kTrkBkg]->AddFriend("tgjMb");
  }  
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFPho,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFJet,  purity, 
  ...
  TH1D* hFFPho_type2 = new TH1D(Form("ffPhoton_type2_icent%d",icent),";p_{T}^{trk}/p_{T}^{photon};dN/dp_{T}",20,0,2);
  corrFunctionTrk* cFFPho_type2 = new corrFunctionTrk();
  TString trkVarFF_type2    = Form("pt/photonEt");
  gammaTrkSingle( gjSpec,  tJet, cFFPho_type2,  purity,
                  sampleType, trkVarFF_type2, trkCutFF,  "trkWeight",
                  phoCandCut, phoDecayCut,  hFFPho_type2, outName, jetMixing);
  
  TH1D* hFFJet_type2 = new TH1D(Form("ffJet_type2_icent%d",icent),";p_{T}^{trk}/p_{T}^{photon};dN/dp_{T}", 20,0,2);
  corrFunctionTrk* cFFJet_type2 = new corrFunctionTrk();
  TString trkVarFFJet_type2      = Form("pt/injPt");
  gammaTrkSingle( gjSpec,  tJet, cFFJet_type2,  purity,
                  sampleType, trkVarFFJet_type2, trkCutFF,  "trkWeight",
                  phoCandCut, phoDecayCut,  hFFJet_type2, outName, jetMixing);

  const int nTrkPtBins = 12;
  double trkPtBins[nTrkPtBins+1] = {1,2,3,4,5,7,9,12,15,20,25,50,100};
 
  TH1D* hFF_type3 = new TH1D(Form("ffPhoton_type3_icent%d",icent),";;",nTrkPtBins,trkPtBins);
  corrFunctionTrk* cFF_type3 = new corrFunctionTrk();
  TString trkVarFF_type3    = Form("pt");
  gammaTrkSingle( gjSpec,  tJet, cFF_type3,  purity,
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFRes03,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFRes05,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFResInc,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFResIncPtCut2,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFResIncPtCut4,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFResIncPtCut8,  purity, 
  ...
  gammaTrkSingle( gjSpec,  tJet, cFFResIncPtCut16,  purity, 
...
void gammaTrkSingle(     GjSpectra* gjSpec,  multiTreeUtil* tJet[3],   corrFunctionTrk* corr, 
			 ...
			 TH1D* hTemplate,  TString outfName, bool jetMixing)
{
  ...
  tJet[kTrkRaw]->Draw2(corr->Func[kPhoCand][kTrkRaw],  var,  phoCandCut  && cut, theWeight);
  if ( tJet[kTrkBkg]!=0)  
    tJet[kTrkBkg]->Draw2(corr->Func[kPhoCand][kTrkBkg],  var,  phoCandCut  && cut, theWeight);
  
  
  if ( (sampleType==kHIDATA) || (sampleType==kPPDATA) ) {
    tJet[kTrkRaw]->Draw2(corr->Func[kPhoDecay][kTrkRaw], var, phoDecayCut && cut,  theWeight);
    if (tJet[kTrkBkg]!=0)  
      tJet[kTrkBkg]->Draw2(corr->Func[kPhoDecay][kTrkBkg], var, phoDecayCut && cut,  theWeight);
  }
