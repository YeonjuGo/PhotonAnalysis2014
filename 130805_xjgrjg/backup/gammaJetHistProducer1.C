...
void gammaJetHistProducer(sampleType collision = kHIDATA, float photonPtThr=60, float photonPtThrUp=1000000, float jetPtThr=30, int icent =1,  int genLevel=0, int qnch = 0) {
  ...
  if ( collision == kHIMC )       stringSampleType = "pbpbMC";
  else if ( collision == kHIDATA )  stringSampleType = "pbpbDATA";
  else if ( collision == kPPDATA )  stringSampleType = "ppDATA";
  else if ( collision == kPPMC )  stringSampleType = "ppMC";
  ...
  TString fname;
  if ( collision == kHIMC)              fname = fnameAllQcdPho30Ak3; //fnameAllQcdPho50Ak3;
