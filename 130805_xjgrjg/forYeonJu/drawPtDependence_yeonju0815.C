...
void drawPtDependence_yeonju0815(bool drawPbPb=1, int xNorm = 1) {
    ...
    for ( int ipt = 0 ; ipt<nPtBin  ; ipt++) { 
        for ( int iSpecies=0 ; iSpecies<=5 ; iSpecies++){ 
            ...
            if ( iSpecies == kPPDATA) { 
                icent = 7;
                fff[iSpecies][iglb] = new TFile(Form("photonTrackCorr_ppDATA_output_photonPtThr%d_to_%d_jetPtThr30_20130815.root",(int)ptBin[ipt],(int)ptBin[ipt+1]));
            }
            if ( iSpecies == kHIDATA) { 
                icent = 10030;
    ...
    TCanvas* c1 = new TCanvas("c1","",1350,400);
    ...
    for ( int ipt = 0 ; ipt<nPtBin  ; ipt++) {
        ...
        if (xNorm==1)     hxjgNorm[iSpecies][iglb]->SetAxisRange(-0.015,0.5,"Y");
        ...
        TH1D* temp1 = new TH1D("temp1","temp1", 100, 0, 100);
        ...
        if (drawPbPb)    l1->AddEntry(temp1,"PbPb 0-30%", "p");
    ...
    for ( int ipt = 0 ; ipt<nPtBin  ; ipt++) {
        ...
        hdphi[iSpecies][iglb]->SetMarkerStyle(20);
        ...
        TH1D* temp2 = new TH1D("temp2","temp2", 100, 0, 100);
        temp2 -> SetMarkerColor(2);
        temp2 -> SetMarkerStyle(20);
        TH1D* temp3 = new TH1D("temp3","temp3", 100, 0, 100);
        ...
        if (drawPbPb)    l1->AddEntry(temp2,"PbPb 0-30%","p");
        if (drawPbPb)    l1->AddEntry(temp3,"PbPb 30-100%","p");
        if ( ipt==0 )   l1->Draw();
