...
void fakerate(){
    ...
    TFile* readFile = new TFile("../files/yskimfiles/yskim_pA_Pyquen_allQCDPhoton_hiForest2_v85.root" ,"READ");
    ...
    //
    int nJetmax = 100;
    float refPt[nJetmax], hf4Sum, pt[nJetmax], eta[nJetmax], dphi[nJetmax];
  ...
  /*  for(int k=0; k<entries; k++){
        ...
        yJet -> GetEntry(k);
        tgj  -> GetEntry(k);
        if ((evtImb.hf4Sum > 0 ) && ( evtImb.hf4Sum < 20 )) {icent = 1;}
        else if (( evtImb.hf4Sum > 20 ) && ( evtImb.hf4Sum < 30 )) {icent = 2;} 
        else if (( evtImb.hf4Sum > 30 ) && ( evtImb.hf4Sum < 100 )) {icent = 3;}
        else continue;
     //   cout << icent << ",   " << evtImb.hf4Sum << endl;
        for(int j=0; j<nJet; j++){
            i = (int)pt[j]/5. - 4; 
           // cout << "pt = " << pt[j] << ", bin = " << i << endl;
            if( i >= nPtBin ) continue;
            if ((abs(eta[j]) < 1.6) && (dphi[j] > 7*3.141592/8.0) && (pt[j] >= ptBin[i] && pt[j] < ptBin[i+1]) && (evtImb.hf4Sum > centBinPa[icent-1] && evtImb.hf4Sum <= centBinPa[icent])){
    ...
    for (int icent = 1 ; icent <= nCentBinPa ; icent++){
        for (int i = 0; i<nPtBin ; i++){ 
            ...
            cout << " fakerate = " << fakerate[icent][i] << ", realjet = " << realjet[icent][i]<< ", totaljet = " << totaljet[icent][i] << endl;
        }
    }

    TCanvas* c_fake = new TCanvas("c_fake", "fake rate", 400, 300); 
    TH1D* h_fake = new TH1D("h_fake",";JetP_{T} (GeV); Fake Rate ",1000,20,200);
...
//
    ...
    yJet -> Draw("pt/refPt >> jetEscale" , "refPt>0 && abs(eta) < 1.6 && dphi > 7*3.141592/8.0");
    jetEscale -> Draw();
    ...
    for(int icent=1; icent <= nCentBinPa ; icent++){
        for(int i=0; i < nPtBin ; i++){
            ...
            yJet -> Draw(Form("pt/refPt >> Escale%d_%d",icent,i) , Form("abs(eta) < 1.6 && dphi > 7*3.141592/8.0 && (pt >= %d && pt < %d) && (hf4Sum > %f && hf4Sum <= %f) ", (int)ptBin[i], (int)ptBin[i+1], (float)centBinPa[icent-1], (float)centBinPa[icent]));
            Escale[icent][i] -> Draw(); 
           ...
           // ((icent==1)||(icent==4))? dx1=0.15 : dx1=0 ;
            ...
            if ( i+1 == nPtBin ) 
                drawText(Form("p_{T}^{#gamma} > %dGeV, ", (int)ptBin[i]), 0.12+dx1,0.84,1,15);//yeonju 130823
            else
                drawText(Form("%dGeV < p_{T}^{#gamma} < %dGeV, ", (int)ptBin[i], (int)ptBin[i+1]), 0.12+dx1,0.84,1,15);//yeonju 130823
    ...
    TH1D* h = new TH1D("h",";JetP_{T} (GeV); Energy Scale ",1000,20,200);
