...
void fitResolandScale(int collision = 4, int flvOpt = 0, int genOpt = 1){
...
*/
   ...
   //     yJet -> addFile("/home/goyeonju/recent2013/jetAnalysis/files/yskimfiles/testOldForest_yskim_PbPb_pythiaHYDJET_forest_AllQCDPhotons30to50.root", "yJet", "");
   //     yJet -> addFile("/home/goyeonju/recent2013/jetAnalysis/files/yskimfiles/testOldForest_yskim_PbPb_pythiaHYDJET_forest_AllQCDPhotons50to80.root", "yJet", "");
   //     yJet -> addFile("/home/goyeonju/recent2013/jetAnalysis/files/yskimfiles/testOldForest_yskim_PbPb_pythiaHYDJET_forest_AllQCDPhotons80to9999.root", "yJet", "");
   ...
   // to find out the role of weighting factor, I removed the values. 
   //     yJet -> addFile("/home/jazzitup/forestFiles/yskimmedFiles/yskim_qcdAllPhoton30to50_genPhotonPtCut40_allCent.root", "yJet", "");
   //     yJet -> addFile("/home/jazzitup/forestFiles/yskimmedFiles/yskim_qcdAllPhoton50to80_genPhotonPtCut40_allCent.root", "yJet", "");
   //     yJet -> addFile("/home/jazzitup/forestFiles/yskimmedFiles/yskim_qcdAllPhoton80to9999_genPhotonPtCut40_allCent.root", "yJet", "");  
    } else if (collision ==3){
        // pA
    ...
    for(int i=0; i < nPtBin ; i++){
        ...
        if ( genOpt == 0 )  {
            yJet -> Draw2(Escale[icent][i], "pt/refPt", centCut && partonCut && Form(" (abs(eta) < 1.6) && (dphi > 7*3.141592/8.0) && (refPt >= %d && refPt < %d)", (int)ptBin[i], (int)ptBin[i+1]), "ptHatWeight"); 
//            yJet -> Draw2(Escale[icent][i], "pt/refPt", centCut && partonCut && Form(" (abs(eta) < 1.6) && (dphi > 7*3.141592/8.0) && (refPt >= %d && refPt < %d)", (int)ptBin[i], (int)ptBin[i+1]), ""); 
            ...
            yJet -> Draw2(Escale[icent][i], "pt/refPt", centCut && partonCut && Form(" (abs(eta) < 1.6) && (dphi > 7*3.141592/8.0) && (pt >= %d && pt < %d) ", (int)ptBin[i], (int)ptBin[i+1]), "ptHatWeight");
            //yJet -> Draw2(Escale[icent][i], "pt/refPt", centCut && partonCut && Form(" (abs(eta) < 1.6) && (dphi > 7*3.141592/8.0) && (pt >= %d && pt < %d) ", (int)ptBin[i], (int)ptBin[i+1]), "tgj.vtxCentWeight");
        }
        //Escale[icent][i] -> Draw(); 
        ...
        cout << "mean : "<< mean[icent][i]<< ", var : " << var[icent][i] << ", resolmean : " << resol[icent][i] << ", resolVar : " << resolVar[icent][i] << endl;
//var[icent][i] =1 ;
//resol[icent][i] = 1;
//resolVar[icent][i] = 1 ;
    ...
    TFile* outFile = new TFile(Form("resolutionHisto_recent%d.root", collision),"RECREATE");
...
//  if(collision==2) cout << " resol C : " << param[0] << ", S : " << param[1] << ", N : : " << param[2] << endl;
 ...
 //  else cout << " N : " << param[0] << endl;
    ...
    c3 -> SaveAs(Form("./graph/ptHatWeight_CSnotFix_scale_resolution_fit_collision%d.pdf",collision));
    //    c_fake -> SaveAs(".gif");
}
