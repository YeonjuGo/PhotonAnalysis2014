...
void calculCrossSection(int collision=1){
  ...
  //  gStyle -> SetFrameBorderSize(0);
    ...
    if(collision==1){//pA data
        ff = new TFile("/home/goyeonju/recent2013/jetAnalysis/files/yskimfiles/pA/yskim_pA_photonSkimForest_v85_HLT_PAPhoton30_NoCaloIdVL_v1_highPtPhoton40.root");
...
//
    ...
    TH1D* effiH = (TH1D*) effiF -> Get("hRatio2");
    ...
    TH1D* effiHpp = (TH1D*) effiFpp -> Get("hRatio2");
...
//
    tpho -> Draw("corrPt >> hdata", centCut && dataCut);
    tphopp -> Draw("corrPt >> hdata_pp","abs(eta) < 1.44 && hadronicOverEm < 0.1 && (ecalRecHitSumEtConeDR04 < 4.2 && hcalTowerSumEtConeDR04 < 2.2 && trkSumPtHollowConeDR04 < 2)" );
    ...
    hdata_pp -> SetLineColor(1);
    hpucorr -> SetLineColor(2);
    hpucorr_pp -> SetLineColor(2);
    hefficorr -> SetLineColor(3);
    hefficorr_pp -> SetLineColor(3);
